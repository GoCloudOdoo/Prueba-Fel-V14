...
def set_data_for_invoice_abono(self):
        ...
        if not api:
            return False
        emi = ET.SubElement(dem, "{" + xmlns + "}Emisor", AfiliacionIVA="GEN", CodigoEstablecimiento=api.code_est, CorreoEmisor=self.company_id.email, NITEmisor=self.company_id.vat, NombreComercial=self.company_id.name, NombreEmisor=self.company_id.company_registry)
        ...
        for line in invoice_line:
            ...
            ET.SubElement(item, "{" + xmlns + "}Descripcion").text = line.product_id.default_code + " | " + line.product_id.name or " "
